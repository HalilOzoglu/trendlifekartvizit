...
// Sabit butonlar (her kartta görünür)
const FIXED_BUTTONS = [
...
export default function CardView({ card, baseUrl }: Props) {
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#fff", fontFamily: "'Segoe UI', system-ui, sans-serif", position: "relative", overflowX: "hidden" }}>
      ...
      {/* Logo / Şirket Adı */}
      <div style={{ background: "#fff", padding: "28px 24px 16px", textAlign: "center", borderBottom: "1px solid #f0f0f0" }}>
        {card.cover_url ? (
      ...
      {/* Video */}
      {card.video_url && (
        <div style={{ margin: "0 16px 0", borderRadius: 12, overflow: "hidden", aspectRatio: "16/9", background: "#000" }}>
          <iframe
      ...
      <div style={{ textAlign: "center", padding: "20px 24px 8px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
          ...
          <svg width="22" height="22" viewBox="0 0 24 24" fill="#1DA1F2">
            <path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" stroke="#1DA1F2" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
            <path d="M9 12l2 2 4-4" stroke="#1DA1F2" strokeWidth="2.5" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </div>
        {card.title && <div style={{ fontSize: "0.95rem", color: "#555", marginTop: 4 }}>{card.title}</div>}
      </div>

      {/* Sabit 6 buton (3x2 grid) */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, padding: "16px 20px" }}>
        {FIXED_BUTTONS.map(btn => (
          <button
            ...
            style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, padding: "16px 8px", borderRadius: 18, background: btn.bg, border: "none", cursor: "pointer", transition: "transform 0.15s", color: "#fff" }}
          ...
          >
            ...
            <span style={{ fontSize: "0.7rem", fontWeight: 600, color: "#fff" }}>{btn.label}</span>
          </button>
        ))}
      </div>

      {/* Sosyal medya butonları (kaydırmalı, 3'lü) */}
      {socialLinks.length > 0 && (
        <div style={{ display: "flex", alignItems: "center", padding: "0 8px 16px", gap: 4 }}>
          <button
            ...
            style={{ background: "none", border: "none", cursor: socialPage === 0 ? "default" : "pointer", color: socialPage === 0 ? "#ccc" : "#333", fontSize: "1.2rem", padding: "0 4px", flexShrink: 0 }}
          >‹</button>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, flex: 1 }}>
            {visibleSocial.map(link => (
              <a
                ...
                style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, padding: "16px 8px", borderRadius: 18, background: SOCIAL_COLORS[link.platform] ?? "#555", textDecoration: "none", color: "#fff", transition: "transform 0.15s" }}
              >
                <div style={{ width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <button
            ...
            style={{ background: "none", border: "none", cursor: socialPage >= totalSocialPages - 1 ? "default" : "pointer", color: socialPage >= totalSocialPages - 1 ? "#ccc" : "#333", fontSize: "1.2rem", padding: "0 4px", flexShrink: 0 }}
          >›</button>
        </div>
      )}

      {/* Alt bar: Fatura | Dil | Banka */}
      <div style={{ position: "sticky", bottom: 0, background: "#fff", borderTop: "1px solid #eee", display: "flex", alignItems: "center", zIndex: 10 }}>
        <button
          onClick={() => setShowInvoice(true)}
          style={{ flex: 1, padding: "14px 8px", background: "#222", color: "#fff", border: "none", cursor: "pointer", fontSize: "0.85rem", fontWeight: 600, borderRadius: "0 0 0 0" }}
        >
          Fatura Bilgileri
        </button>
        <button style={{ padding: "14px 16px", background: "#c0392b", border: "none", cursor: "pointer", fontSize: "1.1rem" }}>
          🇹🇷
        </button>
        <button
          onClick={() => setShowBank(true)}
          style={{ flex: 1, padding: "14px 8px", background: "#222", color: "#fff", border: "none", cursor: "pointer", fontSize: "0.85rem", fontWeight: 600 }}
        >
          Banka Bilgileri
        </button>
      </div>
...
function BottomSheet({ title, children, onClose }: { title: string; children: React.ReactNode; onClose: () => void }) {
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
      ...
      <div style={{ position: "relative", background: "#fff", borderRadius: "20px 20px 0 0", padding: "24px 20px 40px", maxWidth: 480, width: "100%", margin: "0 auto" }}>
...
function YoutubeIcon() {
  return <svg width="28" height="28" viewBox="0 0 24 24" fill="#fff"><path d="M22.54 6.42a2.78 2.78 0 0 0-1.95-1.96C18.88 4 12 4 12 4s-6.88 0-8.59.46a2.78 2.78 0 0 0-1.95 1.96A29 29 0 0 0 1 12a29 29 0 0 0 .46 5.58A2.78 2.78 0 0 0 3.41 19.6C5.12 20 12 20 12 20s6.88 0 8.59-.46a2.78 2.78 0 0 0 1.95-1.95A29 29 0 0 0 23 12a29 29 0 0 0-.46-5.58z"/><polygon points="9.75 15.02 15.5 12 9.75 8.98 9.75 15.02" fill="#FF0000"/></svg>;
}
function TiktokIcon() {
