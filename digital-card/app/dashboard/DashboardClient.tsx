...
import type { Card, SocialLink } from "@/lib/types";
...
export default function DashboardClient({ initialCard, userId }: Props) {
  ...
  const [socialLinks, setSocialLinks] = useState<Partial<SocialLink>[]>(initialCard?.social_links ?? []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showToast(err?.message?.includes("unique") ? "Bu URL adı zaten kullanılıyor" : "Kayıt sırasında hata oluştu");
    } finally {
      setSaving(false);
    }
  };
      ...
      <div style={{ maxWidth: 640, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
          <div>
            ...
            {card.username && (
              <a href={`/${card.username}`} target="_blank" style={{ fontSize: "0.8rem", color: "#e94560" }}>
                /{card.username} →
              </a>
            )}
          </div>
        ...
        <Section title="Kişisel Bilgiler">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <Field label="URL Adı (username)" value={card.username ?? ""} onChange={f("username")} placeholder="uzeyir" />
          ...
          <Field label="Kapak Fotoğrafı URL" value={card.cover_url ?? ""} onChange={f("cover_url")} placeholder="https://..." />
        </Section>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ background: "#1a1a2e", borderRadius: 16, border: "1px solid rgba(255,255,255,0.08)", padding: 20, marginBottom: 16 }}>
      <h2 style={{ fontSize: "0.9rem", fontWeight: 600, color: "rgba(255,255,255,0.6)", marginBottom: 16, textTransform: "uppercase", letterSpacing: "0.5px" }}>{title}</h2>
      {children}
    </div>
  );
}
